...
class proAnWatchFaceView extends Ui.View {
	...
	// App "refresh" timers and license check timer (demo)
    var mUpdateTimer		= null;
    var refreshInterval		= null;
//    var demoTimer			= null; // used if I implement licensing
    ...
    function initialize() {
		...
		// refreshInterval is refresh rate in milliseconds
		refreshInterval = 1000; // refresh rate in milliseconds
        mUpdateTimer = new Timer.Timer();
        mUpdateTimer.start(method(:requestUpdate), refreshInterval, true);
    ...
    function onUpdate(dc) {
	    ...
	    appStore_license		= App.getApp().getProperty("license_prop");
		if (appStore_license.equals(LICENSE)) {
			licensedApp = true;
		}
		/* // not requiring license yet
				// Check if user has entered license code
				if (!licensedApp) {
					demoTimer++;
					if (demoTimer > 30) {
						dc.setColor(c_primary, c_screen);
						dc.clear();
						dc.drawText(mid_x, mid_y, Gfx.FONT_XTINY, "Purchase license\nfor full functionality.\nInstructions in description\nof App in Garmin store.", Gfx.TEXT_JUSTIFY_CENTER|Gfx.TEXT_JUSTIFY_VCENTER);		
						return;
					}
				}
		*/
		...
		/////////////////////////////////////////////////////////////////		
		var currTimer = Sys.getTimer();
    	var currLatency = currTimer - timer;
    	var timeRunning = currTimer - appStartTimer;
    	var timeRunHrs = timeRunning/1000.0/60.0/60.0; // ms->sec->min->hrs
    	var battUsed = appBattStartValue - battery; // battery consumption since App initialized
    	var battPerHr = battUsed / timeRunHrs; // battery usage per hour
    	var battRemaining = (battPerHr > 0) ? (battery / battPerHr) : null;
		...
		if (buttonSelect) {
			...
			var yinc = 15;
			dc.setColor(Gfx.COLOR_BLACK, Gfx.COLOR_TRANSPARENT);
			dc.fillRectangle(tx, ty, mid_x*1.50, mid_y*1.50);
			dc.setColor(Gfx.COLOR_WHITE, Gfx.COLOR_TRANSPARENT);
			tx+=5;
			ty+=5;
			dc.drawText(tx, ty, smallFont, "start batt: " + appBattStartValue.format("%03.2f")+"%", Gfx.TEXT_JUSTIFY_LEFT);
			ty+=yinc;			
			dc.drawText(tx, ty, smallFont, "curr batt: " + battery.format("%03.2f")+"%", Gfx.TEXT_JUSTIFY_LEFT);			
			ty+=yinc;
			dc.drawText(tx, ty, smallFont, "curr latency: " + currLatency, Gfx.TEXT_JUSTIFY_LEFT);
			ty+=yinc;			
			dc.drawText(tx, ty, smallFont, "avg latency: " + appAvgLatency, Gfx.TEXT_JUSTIFY_LEFT);
			ty+=yinc;			
			dc.drawText(tx, ty, smallFont, "mins since start: " + (timeRunning/1000.0/60.0).format("%04.2f"), Gfx.TEXT_JUSTIFY_LEFT);			
			ty+=yinc;			
			dc.drawText(tx, ty, smallFont, "refresh count: " + appRefreshCounter, Gfx.TEXT_JUSTIFY_LEFT);
			ty+=yinc;
			dc.drawText(tx, ty, smallFont, "batt per hour: " + battPerHr.format("%3.2f")+"%", Gfx.TEXT_JUSTIFY_LEFT);
			if (battRemaining != null) { 
				ty+=yinc;
				dc.drawText(tx, ty, smallFont, "apprx hrs left: " + battRemaining.format("%3.1f"), Gfx.TEXT_JUSTIFY_LEFT);
			}
			ty+=(yinc*1.5);
			dc.drawText(mid_x, ty, smallFont, "press start to return", Gfx.TEXT_JUSTIFY_CENTER);			
		}    	
    	appRefreshCounter++;
...
*/
	...
	function drawGauge (dc, x, y, label, metric, fill) {
		...
		if (appStore_gaugeType == gaugeDialW) {
			...
			colOutline	= (appStore_schemeTheme==schemeBlack) ? Gfx.COLOR_LT_GRAY : Gfx.COLOR_BLACK;
			...
			colOutline	= (appStore_schemeTheme==schemeBlack) ? Gfx.COLOR_WHITE : Gfx.COLOR_DK_GRAY;
